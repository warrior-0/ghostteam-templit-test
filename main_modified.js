...
async function getStoriesWithLikes() {
  const storiesSnapshot = await getDocs(collection(db, "urbanStories"));
  const likesSnapshot = await getDocs(collection(db, "urbanLikes"));

  const likesMap = {};
  likesSnapshot.forEach(doc => {
    const data = doc.data();
    likesMap[data.storyId] = (likesMap[data.storyId] || 0) + 1;
  });

  const storiesWithLikes = [];
  storiesSnapshot.forEach(doc => {
    const data = doc.data();
    const id = doc.id;
    storiesWithLikes.push({
      id,
      ...data,
...
document.addEventListener("DOMContentLoaded", () => {
  const popularContainer = document.querySelector("#popular-stories");
  if (!popularContainer) return;

  getStoriesWithLikes().then(stories => {
    const topStories = stories.sort((a, b) => b.likeCount - a.likeCount).slice(0, 5);
    topStories.forEach(story => {
      const div = document.createElement("div");
      div.innerHTML = `<h3>${story.title}</h3><p>좋아요: ${story.likeCount}</p>`;
