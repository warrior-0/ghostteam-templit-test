    ...
    <div class="product-card urban-item urban-detail" style="width:100%;max-width:1200px;margin:0 auto; position: relative;">
      ...
      <div class="voice-mode" style="position:absolute; top:1rem; right:1rem;">
        ...
        <audio id="urbanVoiceAudio" style="display:none; margin-top:0.5rem; width:100%;">
          <source src="audio/audio/urban${id}.mp3" type="audio/mpeg">
...
async function getStoryLikes(storyId) {
  ...
  likesSnapshot.forEach(doc => {
    if (doc.data().storyId === storyId) count++;
  });
  return count;
}

async function updateStoryDetail() {
  const storyId = getParamFromURL("id");
  if (!storyId) return;

  const docRef = doc(db, "urbanStories", storyId);
  const docSnap = await getDoc(docRef);

  if (docSnap.exists()) {
    const data = docSnap.data();
    const likeCount = await getStoryLikes(storyId);

    document.querySelector("#story-title").textContent = data.title;
    document.querySelector("#story-body").textContent = data.body;
    document.querySelector("#story-detail").textContent = data.detail;
    document.querySelector("#story-likes").textContent = `좋아요: ${likeCount}`;
  }
}

document.addEventListener("DOMContentLoaded", updateStoryDetail);
